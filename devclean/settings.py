...
def load_config(config_path: Optional[Path] = None) -> DevCleanConfig:
    ...
    try:
        ...
        if 'scan' in data:
            ...
            config.scan = ScanSettings(
                ...
                include_node_modules=scan_data.get('include_node_modules', config.scan.include_node_modules),
        ...
        if 'safety' in data:
            ...
            config.safety = SafetySettings(
                require_confirmation=safety_data.get('require_confirmation', config.safety.require_confirmation),
                protected_paths=safety_data.get('protected_paths', config.safety.protected_paths),
                never_delete_patterns=safety_data.get('never_delete_patterns', config.safety.never_delete_patterns),
                always_safe_patterns=safety_data.get('always_safe_patterns', config.safety.always_safe_patterns),
            )
        
        # Top-level settings
        config.additional_search_paths = data.get('additional_search_paths', config.additional_search_paths)
...
def save_config(config: DevCleanConfig, config_path: Optional[Path] = None) -> None:
...
def get_api_key_from_config_or_env(config: DevCleanConfig) -> Optional[str]:
