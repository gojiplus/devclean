...
from .config import CRUFT_PATTERNS, VENV_NAMES, VENV_SEARCH_DIRS, CruftPattern
...
class CruftItem:
    ...
    description: str
    safe: bool = True
    tool_installed: bool | None = None  # None = unknown/not checked
...
def scan_known_cruft(home: Path, min_size_mb: int = 100, max_workers: int = 4) -> list[CruftItem]:
    ...
    def scan_pattern(pattern: CruftPattern) -> CruftItem | None:
