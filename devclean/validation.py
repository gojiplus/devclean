...
from .exceptions import UnsafePathError, PathNotFoundError
...
def validate_directory_for_deletion(path: Path, protected_paths: Optional[List[str]] = None) -> None:
        ...
        try:
            ...
            if path.is_relative_to(protected_path):
                ...
                if protected_path == home:
                    ...
                    special_subdirs = {"Documents", "Desktop", "Downloads", "Pictures", "Music", "Movies"}
                    if path.name in special_subdirs or any(part in special_subdirs for part in path.parts):
                        raise UnsafePathError(f"Cannot delete special directory: {path}")
                else:
                    raise UnsafePathError(f"Cannot delete path under protected directory {protected_path}: {path}")
