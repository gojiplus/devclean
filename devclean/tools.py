...
from .scanner import scan_all, get_dir_size, ScanResult, CruftItem
from .exceptions import DeletionError, UnsafePathError, PathNotFoundError
    ...
    if name == "scan_disk":
        ...
        try:
            ...
            if result.returncode == 0:
                return f"{tool}: INSTALLED at {result.stdout.decode().strip()}"
                ...
                if result.returncode != 0:
                    ...
                    if "Operation not permitted" in error_msg:
                        return json.dumps({
                            ...
                            "message": f"Even sudo can't delete this — macOS sandbox protection. Terminal needs Full Disk Access.",
                            "path": str(path),
        ...
        if deleted_paths:
            ...
            result_lines.append(f"✅ Successfully deleted {len(deleted_paths)} items, freed {freed_gb:.1f} GB:")
            for path in deleted_paths[:5]:  # Show first 5
